...
## オーナー設定の記録（外部設定は迷わない粒度で。UI が指示と違っても混乱させない）

オーナー（非エンジニア）にしかできない外部設定（API 鍵・OAuth・デプロイ・決済など）は、
`OWNER-SETUP.md`（無ければ新規作成。LMS は `SETUP.md` があるのでそこへ追記でも可）に
「気力・体力が無くても上から順にやれば終わる」粒度で書く。各社の管理画面はボタン名・配置を
頻繁に変えるため、**指示と実画面が食い違うとオーナーは混乱する**。オーナー向け手順は必ず次を守る
（全プロダクト共通の原則。bonds/cares/stock-screener/vm-suite/zentrack でも同一方針）:

1. 各手順の先頭に「▼ ねらい（この操作で何を達成したいか）」を1行。名前でなく目的で探せるようにする。
2. ボタン名は「（または〜/英語名）」で別名を併記する。
3. 迷ったとき用に【検索キーワード】と直リンク URL を添える。
4. 各手順に「画面が違うとき」の注記を付け、最後は必ず「分からなければ止めて画面の写真を送って相談」に逃がす。
5. 秘密の値（Secret）と公開値（環境変数/Variables/管理画面）のどちらに入れるかを明示し、取り違えを防ぐ。
6. 「終わるまで機能は準備中で縮退し、アプリは壊れない」ことを明記して不安を取り除く。

Claude はオーナー設定が要る機能を作ったら、同じ回で SETUP.md/OWNER-SETUP.md にこの粒度で追記し URL を伝える。
UI が指示と違うという申告があれば、実画面に合わせて手順を書き直す。

**LMS のオーナー設定（把握しているもの・詳細は SETUP.md 参照/拡充）**:
①Firebase 設定（管理画面 `admin/config` または `admin/secrets` 経由で投入。コードにハードコードしない）
