...
var TimeMarketplace = {
  ...
  renderWidget() {
    ...
    let html = `<div class="marketplace-widget">
      <h3>今週の空き時間</h3>
      <div class="mp-stats">
        ${Components.statCard('空き時間', free.totalHours + '時間', null, null)}
        ${Components.statCard('空き枠', free.slots + '枠', null, null)}
        ${Components.statCard('見込み収入', free.potentialRevenue.toLocaleString() + '円', null, null)}
      </div>`;
    ...
    if (slots.length === 0) {
      ...
      slots.forEach(slot => {
        ...
        html += `<div class="booking-slot">
          ...
          <button class="btn btn-primary btn-sm" data-start="${Components.escapeHtml(slot.start)}" data-end="${Components.escapeHtml(slot.end)}" onclick="TimeMarketplace.requestBooking(this.dataset.start, this.dataset.end)">予約する</button>
        </div>`;
      });
    }
