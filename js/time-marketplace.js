    ...
    if (slots.length === 0) {
      ...
      slots.forEach(slot => {
        ...
        html += `<div class="booking-slot">
          ...
          <button class="btn btn-primary btn-sm" data-start="${slot.start}" data-end="${slot.end}" onclick="TimeMarketplace.requestBooking(this.dataset.start,this.dataset.end)">予約する</button>
        </div>`;
      });
    }
