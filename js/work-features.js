...
むずかしい言葉は使わないでください。`;
  ...
  async diagnose() {
    ...
    const profile = store.get('userProfile') || {};
    const ageLabel = profile.age ? `${profile.age}歳` : '65歳前後';
    const genderLabel = profile.gender === 'male' ? '男性' : profile.gender === 'female' ? '女性' : '';
    const personaLine = genderLabel
      ? `${ageLabel}の${genderLabel}からご相談です。`
      : `${ageLabel}の方からご相談です。`;
    ...
    const prompt = `${personaLine}この方に合う活動を3つ見つけてください。

やってきたこと：${experience}
好きなこと：${skills}
使える時間：${time}
やり方：${mode === 'online' ? '家からオンライン' : mode === 'offline' ? '直接会って' : 'どちらでも'}
希望する形：${types.length ? types.join('、') : 'どんな形でも'}
    ...
    if (mpSettings.enabled) {
      ...
      return `<div class="time-selling-banner active">
        ...
        <div class="tsb-content">
          <h4>空き時間販売 稼働中</h4>
          <p>今週の空き：${free.totalHours}時間${free.potentialRevenue > 0 ? ' ｜ 見込み収入：' + free.potentialRevenue.toLocaleString() + '円' : ''}</p>
        </div>
