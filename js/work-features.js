...
むずかしい言葉は使わないでください。`;
  ...
  async diagnose() {
    ...
    const aiExp = document.getElementById('diagAI')?.value || '';
    ...
    if (!experience && !skills) {
      Components.showToast('ご経験やお得意なことを教えてください', 'info');
      return;
    }
    ...
    if (prefs.bookkeeping) types.push('活動記録');

    const prompt = `以下の方に合う活動を3つ提案してください。
...
${income ? '【目標収入】' + income : ''}
${aiExp ? '【デジタルツール経験】' + aiExp : ''}
【希望する形】${types.join('、') || 'なんでもOK'}

それぞれについて：
1. 何をするか（ひと言で）
2. 具体的な始め方（今日15分でできること）
3. どこで始めるか（サイト名・場所）
4. どんないいことがあるか
