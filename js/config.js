  ...
  domains: {
    ...
    health: {
      ...
      dataFields: {
        ...
        recipes: [
          { key: 'recipe_id',    type: 'text',     label: 'recipe_id' },
          { key: 'title',        type: 'text',     label: 'recipe_title' },
          { key: 'source',       type: 'text',     label: 'recipe_source' },
          { key: 'source_url',   type: 'text',     label: 'recipe_url' },
          { key: 'servings',     type: 'number',   label: 'servings' },
          { key: 'cook_minutes', type: 'number',   label: 'cook_minutes' },
          { key: 'calories',     type: 'number',   label: 'calories' },
          { key: 'ingredients',  type: 'textarea', label: 'ingredients_json' },
          { key: 'steps',        type: 'textarea', label: 'steps_json' },
          { key: 'image_url',    type: 'text',     label: 'image_url' },
          { key: 'tags',         type: 'text',     label: 'tags' },
          { key: 'rating',       type: 'slider',   min: 0, max: 5, label: 'rating' }
        ],
        mealPlans: [
          { key: 'week_start_date', type: 'date',     label: 'week_start_date' },
          { key: 'plan',            type: 'textarea', label: 'plan_json' },
          { key: 'note',            type: 'textarea', label: 'note' },
          { key: 'generated_by',    type: 'text',     label: 'generated_by' }
        ],
        shoppingLists: [
          { key: 'plan_id',          type: 'text',     label: 'plan_id' },
...
難しい言葉は使わず、やさしい日本語で。日本語で回答。`
    ...
    health_menuPlan: {
      name: '健康 週間献立提案',
      domain: 'health',
      description: '嗜好・健康状態・候補レシピから1週間の献立を組み立てる',
      schedule: 'manual',
      active: true,
      prompt: `あなたは家庭の献立を整えるパートナーです。利用者の嗜好・健康状態・最近7日の体調記録と、候補レシピのリストを受け取ります。働き盛り（平均40〜50代）の家庭を想定し、1週間×3食（朝・昼・夕、計21枠）の献立を返してください。

入力に含まれるもの:
- profile（年齢・世帯人数・好きな料理ジャンル・苦手食材・食事の配慮・予算・調理時間上限・キッチン機器・食の目標）
- allergies（アレルギー）
- recentHealth（直近7日の症状・バイタル・睡眠・運動）
- candidates（候補レシピのJSON配列：title, cook_minutes, calories, ingredients, tags, source_url）

絶対条件:
- アレルギー食材は1食たりとも入れない
- 苦手食材は極力避ける
- 1食あたり cook_minutes が利用者の上限を超えるものは平日朝・昼に置かない（週末夕食は可）
- 1日の塩分・糖質・タンパク質のバランスを意識
- 同じ主菜を連続させない、肉・魚・豆・卵をローテーション
- 3〜4日おきに買い物を想定し、傷みやすい食材は前半に配置
- 残り物・作り置きの活用を1週で2〜3回入れる
- food_health_goal を反映（例: 「血圧ケア」なら塩分2g/食以下を意識）

出力は次のJSONのみ（前後の説明文なし）:
{
  "week_start_date": "YYYY-MM-DD",
  "plan": {
    "monday":    { "breakfast": {"recipe_id":"...", "title":"...", "cook_minutes":N, "calories":N}, "lunch": {...}, "dinner": {...} },
    "tuesday":   {...},
    "wednesday": {...},
    "thursday":  {...},
    "friday":    {...},
  ...
  "rationale": "選定理由（80字以内・利用者向けのやさしい日本語、専門用語禁止）",
  "weekly_nutrition_note": "1週間の栄養バランスの簡潔なメモ（60字以内）"
}

ユーザー向け文言には「AI」「人工知能」「モデル」等の言葉を使わず、「献立」「相談」「整える」などに置き換える。日本語で回答。`
    },

    health_recipeSheet: {
      name: '健康 一枚紙レシピ整形',
      domain: 'health',
      description: 'レシピ1件をA4一枚紙の調理手順に整形',
      schedule: 'manual',
      active: true,
      prompt: `1つのレシピを、A4用紙1枚で読みやすい調理手順に整えてください。働き盛り（40〜50代）の家庭で、片手にスマホを持って読みながら調理する場面を想定。やさしい日本語ですが、子供向けにはせず大人として扱う口調で。

絶対条件:
- 番号付きの手順（最大8ステップ）。1ステップ40字以内。
- 各ステップに「目安時間」「火加減（強火/中火/弱火/余熱）」「並行作業のヒント」を1つ以上含める
- 下ごしらえ（材料の切り方・下味）と本調理を分けて書く
- 火を止めるタイミング、味見のタイミングは明示
- 「AI」「人工知能」等の言葉は禁止。「整える」「合わせる」「仕上げる」を使う

入力（JSON）:
- title, servings, cook_minutes, ingredients[{name, qty, unit, prep}], steps[], notes, source_url

出力（JSONのみ、前後の説明なし）:
{
  "title": "...",
  "subtitle": "副題（30字以内・例: 平日30分で整える、休日のごほうび 等）",
  "servings": N,
  "total_minutes": N,
  "tools": ["フライパン", "鍋", ...],
  "ingredients_grouped": [
    { "group": "主材料", "items": [{"name":"...", "qty":"...", "prep":"..."}] },
    { "group": "調味料", "items": [...] }
  ],
  "prep_steps": ["下ごしらえ手順1", "..."],
  "cook_steps": [
    { "n": 1, "text": "...", "minutes": N, "heat": "中火", "tip": "..." },
    ...
  ],
  "finishing": "盛り付け・仕上げの一言",
  "tips": ["コツ1（30字以内）", "コツ2"],
  "storage": "保存方法と日持ち（30字以内）"
}
...
※投資は自己責任です。必ず専門家にご相談ください。`
  ...
  // ─── Rakuten API (recipes + market search) ───
  // applicationId は admin/secrets で秘匿管理。affiliateId と endpoint は公開可能なので
  // admin/config を経由して全ユーザーへ伝播する。endpoint が空のときは公式ドメインを
  // JSONP 直接呼び出しでフォールバック（meal-planner.js 内で判定）。
  rakuten: {
    applicationId: '',
    affiliateId: '',
    endpoint: ''
  ...
  profileSchema: {
    ...
    lifestyle: [
      ...
      { key: 'food_cuisines', type: 'multiselect',
        options: ['和食', '洋食', '中華', '韓国料理', 'エスニック', 'イタリアン', 'フレンチ'],
        label: '好きな料理ジャンル' },
      { key: 'food_dislikes', type: 'textarea', label: '苦手な食材・避けたい食材' },
      { key: 'food_restrictions', type: 'multiselect',
        options: ['塩分控えめ', '糖質オフ', '低脂質', '高タンパク', 'グルテンフリー', '乳製品なし', '菜食'],
        label: '食事の配慮' },
      { key: 'food_budget', type: 'select',
        options: ['週5000円以下', '週5000〜8000円', '週8000〜12000円', '週12000〜18000円', '週18000円以上'],
        label: '食費予算（週・1人あたり）' },
      { key: 'food_max_minutes', type: 'select',
        options: ['10', '20', '30', '45', '60'],
        label: '1食あたりの調理時間（分・上限）' },
      { key: 'food_kitchen_tools', type: 'multiselect',
        options: ['電子レンジ', 'オーブン', '炊飯器', '圧力鍋', 'ホットクック', 'エアフライヤー', '魚焼きグリル', '土鍋'],
        label: '使えるキッチン機器' },
      { key: 'food_health_goal', type: 'select',
        options: ['特になし', '体重管理', '疲労回復', '筋力維持', '血圧ケア', '胃腸休め', '美容', '集中力'],
        label: '食で目指したいこと' }
    ],
    // 健康
    health: [
