...
【可視化グラフ8枚】ROICツリー/CFMマップ/PL推移/BS推移/CF推移/ROIC・WACC・スプレッド/DCF感度ヒートマップ/スコアカードレーダーチャート`
  ...
  inlinePrompts: {
    text_analysis: {
      ...
      prompt: `ユーザーがメッセージを入力しました。内容がどの領域に関するか判断し、以下の構造でJSONを返してください。

【領域の定義】
・意識 — 気持ち・感情・瞑想・マインドフルネス・人生の意味・価値観・ストレス・幸福感・禅
・健康 — 体の調子・病気・薬・睡眠・食事・運動・健康診断・体重・血圧・疲れ
・時間 — スケジュール・予定・カレンダー・時間の使い方・習慣・タスク・締め切り
・仕事 — 仕事・副業・ボランティア・スキル・キャリア・お仕事探し・収入
・関係 — 家族・友人・知人・コミュニケーション・連絡・人間関係・孤独・誕生日
・資産 — お金・貯金・投資・株・NISA・保険・不動産・家計・節約・収入・支出

【出力ルール（厳守）】
・出力は JSON オブジェクト **のみ**。前後に説明文を書かない。
・マークダウンのコードフェンス（\`\`\`json など）を **使わない**。
・"response" はユーザーに見せるやさしい日本語（共感 + 気づき + 助言、200文字以内）。
・"actions" は具体的な行動提案の文字列配列（1〜3件、各30文字以内）。
・"domains" は該当する領域名の配列。最も関係の強いものを先に書く（例：["資産"]、["健康","時間"]）。
...
【例2】
入力: 最近、眠れない夜が続いています
出力: {"domains":["健康","意識"],"response":"眠れない夜が続いているのはつらいですね。睡眠は心と体の両方に影響しますので、一緒に原因を探ってみましょう。","actions":["寝る前のルーティンを決める","カフェインを午後2時以降は控える"]}`
    },
