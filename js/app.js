...
希望する働き方: ${resume.workStyle}`;
  ...
  addNewPrompt() {
    this.promptModal('プロンプトのキー名', 'work_custom', (key) => {
      if (!key) return;
  ...
  addAdminEmail() {
    this.promptModal('管理者のメールアドレスを入力', 'admin@example.com', async (email) => {
  ...
  generateDemoData() {
    this.confirmModal('デモデータを生成しますか？既存データに追加されます。', () => {
      ...
      const vals = [5, 6, 7, 6, 5, 7, 6];
      const sleeps = [6, 7, 8, 7, 6, 8, 7];
      for (let i = 0; i < 7; i++) {
        const d = new Date(today);
        d.setDate(today.getDate() - i);
        store.addDomainEntry('health', 'symptoms', { condition_level: vals[i], timestamp: d.toISOString() });
        store.addDomainEntry('health', 'sleepData', { quality: sleeps[i], timestamp: d.toISOString() });
      }
  ...
  maybeShowWelcome() {
    if (store.get('_welcomeShown')) return;
    store.set('_welcomeShown', true);
    setTimeout(() => {
      this.openModal('LMSへようこそ', `
        <div style="text-align:center;padding:8px 0 16px;">
          <div style="font-size:48px;margin-bottom:12px;">L</div>
          <h3 style="margin:0 0 8px;font-size:20px;">人生の6領域を、一緒に整えましょう</h3>
          <p style="color:var(--text-secondary);font-size:14px;margin:0 0 24px;line-height:1.7;">
            意識・健康・時間・仕事・関係・資産。<br>
            まず最初の一歩として、今日の体調を記録してみましょう。
          </p>
        </div>
        <div style="display:grid;gap:10px;margin-bottom:20px;">
          <button class="btn btn-primary btn-lg" style="justify-content:center;" onclick="app.closeModal();app.switchDomain('health');app.navigate('record');">
            今日の体調を記録する →
          </button>
          <button class="btn btn-secondary" style="justify-content:center;" onclick="app.closeModal();app.navigate('settings');">
            まずプロフィールを設定する
          </button>
          <button class="btn btn-secondary" style="justify-content:center;" onclick="app.closeModal();">
            後で（ホームに戻る）
          </button>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:12px;text-align:center;color:var(--text-muted);">
          <div style="padding:8px;background:var(--bg-secondary);border-radius:8px;">
            <div style="font-size:20px;margin-bottom:4px;">📝</div>記録する
          </div>
          <div style="padding:8px;background:var(--bg-secondary);border-radius:8px;">
            <div style="font-size:20px;margin-bottom:4px;">💬</div>相談する
          </div>
          <div style="padding:8px;background:var(--bg-secondary);border-radius:8px;">
            <div style="font-size:20px;margin-bottom:4px;">⚡</div>改善する
          </div>
        </div>
      `);
    }, 600);
  ...
  // Mobile-safe replacement for confirm()
  confirmModal(message, onYes, danger = false) {
    const btnClass = danger ? 'btn-danger' : 'btn-primary';
    this.openModal('確認', `
      <p style="margin-bottom:20px;">${Components.escapeHtml ? Components.escapeHtml(message) : message}</p>
      <div class="form-actions">
        <button class="btn ${btnClass}" onclick="app._confirmYes()">はい</button>
  ...
  // Mobile-safe replacement for prompt()
  promptModal(title, placeholder, onSubmit) {
    this.openModal(title, `
      <div class="form-group" style="margin-bottom:16px;">
        <input type="text" id="promptInput" class="form-input" placeholder="${Components.escapeHtml ? Components.escapeHtml(placeholder) : placeholder}"
          onkeydown="if(event.key==='Enter')app._promptSubmit()">
      </div>
      <div class="form-actions">
        <button class="btn btn-primary" onclick="app._promptSubmit()">OK</button>
        <button class="btn btn-secondary" onclick="app.closeModal()">キャンセル</button>
      </div>
    `);
    this._promptCallback = onSubmit;
    setTimeout(() => document.getElementById('promptInput')?.focus(), 80);
  }
