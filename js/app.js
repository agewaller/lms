...
希望する働き方: ${resume.workStyle}`;
  ...
  addNewPrompt() {
    Components.promptDialog('プロンプトのキー名を入力（例: work_custom）', 'work_custom', (key) => {
      if (!key) return;
  ...
  clearFirebaseConfig() {
    Components.confirmDialog('Firebase設定を削除しますか？', () => {
  ...
  addAdminEmail() {
    Components.promptDialog('管理者として追加するメールアドレスを入力してください', 'example@email.com', async (email) => {
  ...
  deleteAllData() {
    Components.confirmDialog('本当にすべてのデータを削除しますか？この操作は元に戻せません。', () => {
      Components.confirmDialog('最終確認：すべてのデータを完全に削除します。よろしいですか？', () => {
        store.clearAll();
