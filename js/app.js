...
希望する働き方: ${resume.workStyle}`;
  ...
  deleteAllData() {
    this.confirmModal('本当にすべてのデータを削除しますか？この操作は元に戻せません。', () => {
      ...
      }, { label: '完全に削除', btnClass: 'btn-danger' });
    }, { label: '削除する', btnClass: 'btn-danger' });
  }
  ...
  // ─── Confirmation Modal (replaces browser confirm() for mobile compat) ───
  confirmModal(message, onConfirm, options = {}) {
    this._confirmCallback = onConfirm;
    const label = Components.escapeHtml(options.label || '削除');
    const btnClass = options.btnClass || 'btn-danger';
    const body = `<p style="font-size:16px;line-height:1.8;margin-bottom:24px;">${Components.escapeHtml(message)}</p>
      <div class="form-actions" style="justify-content:flex-end;">
        <button class="btn btn-secondary" onclick="app.closeModal()">キャンセル</button>
        <button class="btn ${btnClass}" onclick="app.closeModal();app._runConfirm()">${label}</button>
      </div>`;
    this.openModal('確認', body);
  }
