...
希望する働き方: ${resume.workStyle}`;
  ...
  addNewPrompt() {
    Components.promptModal('プロンプトのキー名を入力してください', '例: work_custom', (key) => {
  ...
  clearFirebaseConfig() {
    Components.confirmModal('Firebase設定を削除しますか？再読み込みが必要になります。', () => {
  ...
  async addAdminEmail() {
    Components.promptModal('管理者として追加するメールアドレスを入力してください', 'admin@example.com', async (email) => {
  ...
  generateDemoData() {
    Components.confirmModal('デモデータを生成しますか？既存データに追加されます。', () => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        store.addDomainEntry('health', 'symptoms', { condition_level: 5 + (i % 3), timestamp: ts });
        store.addDomainEntry('health', 'sleepData', { quality: 6 + (i % 3), timestamp: ts });
        store.addDomainEntry('consciousness', 'entries', { mood_level: 5 + (i % 4), gratitude: 'サンプル', timestamp: ts });
      }
