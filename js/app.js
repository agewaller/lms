...
希望する働き方: ${resume.workStyle}`;
  ...
  addNewPrompt() {
    app.openModal('新しいプロンプトを追加', `
      <div class="form-group"><label>キー名（例: work_custom）</label>
        <input type="text" id="newPromptKey" class="form-input" placeholder="work_custom"></div>
      <div style="display:flex;gap:8px;margin-top:16px">
        <button class="btn btn-secondary" onclick="app.closeModal()">キャンセル</button>
        <button class="btn btn-primary" onclick="app._confirmAddPrompt()">追加する</button>
      </div>`);
  }
  ...
  clearFirebaseConfig() {
    Components.showConfirm('Firebase設定を削除しますか？再読み込みが必要になります。', () => {
  ...
  removeAdminEmail(email) {
    ...
    Components.showConfirm(`${Components.escapeHtml(email)} を管理者から外しますか？`, async () => {
  ...
  deleteAllData() {
    Components.showConfirm('すべてのデータを削除しますか？この操作は元に戻せません。', () => {
      Components.showConfirm('最終確認：本当にすべて削除します。よろしいですか？', () => {
        store.clearAll();
        Components.showToast('すべてのデータを削除しました', 'info');
        window.location.reload();
      }, { confirmText: '完全に削除', danger: true });
    }, { confirmText: '次へ', danger: true });
  }
