...
希望する働き方: ${resume.workStyle}`;
  ...
  addNewPrompt() {
    this.openModal('新しいプロンプトを追加', `
      <div class="form-group">
        <label>キー名（英数字・アンダーバーのみ）</label>
        <input type="text" id="newPromptKey" class="form-input" placeholder="例: work_custom">
      </div>
      <button class="btn btn-primary" onclick="app._confirmAddPrompt()">追加</button>
    `);
  }
  ...
  _confirmAddPrompt() {
    ...
    if (!/^[a-z0-9_]+$/.test(key)) { Components.showToast('英小文字・数字・_のみ使用できます', 'error'); return; }
  ...
  async removeAdminEmail(email) {
    ...
    Components.showConfirm(`${email} を管理者から外しますか？`, async () => {
      await this._doRemoveAdminEmail(email);
    });
  }
