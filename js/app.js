...
var App = class App {
  ...
  deleteDataEntry(domain, category, id) {
    this.confirmDialog('この記録を削除しますか？', () => {
      const key = `${domain}_${category}`;
  ...
  fitbitDisconnect() {
    this.confirmDialog('Fitbit接続を解除しますか？', () => {
  ...
  gcalDisconnect() {
    this.confirmDialog('Googleカレンダー接続を解除しますか？', () => {
  ...
  outlookDisconnect() {
    this.confirmDialog('Outlook接続を解除しますか？', () => {
  ...
  gmailDisconnect() {
    this.confirmDialog('Gmail接続を解除しますか？', () => {
...
希望する働き方: ${resume.workStyle}`;
  ...
  deletePrompt(key) {
    this.confirmDialog('このプロンプトを削除しますか？', () => {
  ...
  addNewPrompt() {
    this.promptDialog('プロンプトのキー名を入力（例: work_custom）', 'work_custom', (key) => {
  ...
  clearApiKeys() {
    this.confirmDialog('すべてのAPIキーを削除しますか？', () => {
  ...
  clearFirebaseConfig() {
    this.confirmDialog('Firebase設定を削除しますか？', () => {
  ...
  async addAdminEmail() {
    this.promptDialog('管理者として追加するメールアドレスを入力してください', 'example@email.com', async (email) => {
  ...
  async removeAdminEmail(email) {
    ...
    this.confirmDialog(`${email} を管理者から外しますか？`, async () => {
  ...
  generateDemoData() {
    this.confirmDialog('デモデータを生成しますか？既存データに追加されます。', () => {
  ...
  deleteAllData() {
    this.confirmDialog('本当にすべてのデータを削除しますか？この操作は元に戻せません。', () => {
      this.confirmDialog('最終確認：すべてのデータを完全に削除します。よろしいですか？', () => {
        store.clearAll();
  ...
  // ─── Mobile-safe confirm dialog (replaces window.confirm()) ───
  confirmDialog(message, onConfirm, confirmText, isDanger) {
    confirmText = confirmText || '実行';
    const btnClass = isDanger ? 'btn-danger' : 'btn-primary';
    const self = this;
    this._pendingConfirm = function() {
      self.closeModal();
      onConfirm();
    };
    this.openModal('確認', `
      ...
      <div class="form-actions">
        <button class="btn ${btnClass}" onclick="app._pendingConfirm()">${Components.escapeHtml(confirmText)}</button>
        <button class="btn btn-secondary" onclick="app.closeModal()">キャンセル</button>
      </div>
    `);
  }

  // ─── Mobile-safe prompt dialog (replaces window.prompt()) ───
  promptDialog(message, placeholder, onConfirm) {
    const self = this;
    this._pendingPrompt = function() {
    ...
    this.openModal('入力', `
      <p style="margin-bottom:12px;font-size:16px;">${Components.escapeHtml(message)}</p>
      <input type="text" id="_promptInput" class="form-input" placeholder="${Components.escapeHtml(placeholder || '')}"
        onkeydown="if(event.key==='Enter')app._pendingPrompt()">
      <div class="form-actions" style="margin-top:12px;">
        <button class="btn btn-primary" onclick="app._pendingPrompt()">決定</button>
        <button class="btn btn-secondary" onclick="app.closeModal()">キャンセル</button>
      </div>
    `);
    setTimeout(() => document.getElementById('_promptInput')?.focus(), 100);
  }
