    ...
    birthdayList.forEach(c => {
      ...
      html += `<div class="contact-suggest birthday-suggest">
        ...
        <div class="cs-actions">
          ...
          <button class="btn btn-sm btn-secondary" data-log-name="${Components.escapeHtml(c.name)}" data-log-type="message" onclick="RelationshipFeatures.logContact(this.dataset.logName,this.dataset.logType)">💬 連絡済み</button>
        </div>
      </div>`;
    });
    ...
    overdueList.forEach(d => {
      ...
      html += `<div class="contact-suggest ${d.urgency >= 5 ? 'urgent' : ''}">
        ...
        <div class="cs-actions">
          ...
          <button class="btn btn-sm btn-secondary" data-log-name="${Components.escapeHtml(d.name)}" data-log-type="call" onclick="RelationshipFeatures.logContact(this.dataset.logName,this.dataset.logType)">📝 連絡済み</button>
        </div>
      </div>`;
    });
