    ...
    birthdayList.forEach(c => {
      ...
      html += `<div class="contact-suggest birthday-suggest">
        ...
        <div class="cs-actions">
          ...
          <button class="btn btn-sm btn-secondary" data-name="${Components.escapeHtml(c.name)}" data-type="message" onclick="RelationshipFeatures.logContact(this.dataset.name,this.dataset.type)">💬 連絡済み</button>
        </div>
      </div>`;
    });
    ...
    overdueList.forEach(d => {
      ...
      html += `<div class="contact-suggest ${d.urgency >= 5 ? 'urgent' : ''}">
        ...
        <div class="cs-actions">
          ...
          <button class="btn btn-sm btn-secondary" data-name="${Components.escapeHtml(d.name)}" data-type="call" onclick="RelationshipFeatures.logContact(this.dataset.name,this.dataset.type)">📝 連絡済み</button>
        </div>
      </div>`;
    });
