...
var Components = {
  ...
  renderField(f) {
    ...
    switch (f.type) {
      case 'slider': {
        const _mid = Math.round(((f.min || 0) + (f.max || 10)) / 2);
        return `<div class="slider-field">
          <input type="range" name="${name}" min="${f.min||0}" max="${f.max||10}" value="${_mid}" oninput="this.nextElementSibling.textContent=this.value">
